...
class TestTypeChecks(unittest.TestCase):
    ...
    def test_is_text_and_binary(self):
        self.assertTrue(is_text("caf\xe9"))
        self.assertFalse(is_text(b"bytes"))
        self.assertTrue(is_binary(b"raw"))
        self.assertFalse(is_binary("text"))

    def test_basestring(self):
        self.assertIsInstance("x", str)
        self.assertIsInstance("x", str)
        print("str checks passed")

    def test_register_view(self):
        view = register_view(b"\x00\x01\x02\x03\x04\x05", 2, 3)
        self.assertEqual(bytes(view), b"\x02\x03\x04")
