...
class CacheEntry(object):
    ...
    def _compute_fingerprint(value):
        """SHA-1 fingerprint of pickled value."""
        try:
...
class LRUCache(object):
    ...
    def _bucket_for_key(self, key):
        """Bucket assignment via MD5 hash."""
        key_bytes = key.encode("utf-8") if isinstance(key, str) else key
        hash_val = int(hashlib.md5(key_bytes).hexdigest()[:8], 16)
...
class CacheManager(object):
    """High-level cache with disk persistence."""
    ...
    def _hash_key(raw_key):
        """MD5 hash of key (requires bytes in Py3)."""
    ...
    def flush_to_disk(self):
        """Persist cache entries to disk with pickle protocol 2."""
