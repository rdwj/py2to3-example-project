...
def decode_comp3(raw_bytes):
    ...
    # In Py3, indexing bytes yields int directly -- no ord() needed
    for i in range(len(raw_bytes) - 1):
        high_nibble = (raw_bytes[i] >> 4) & 0x0F
        low_nibble = raw_bytes[i] & 0x0F
...
class MainframeParser:
    ...
    def parse_file(self, file_path):
        ...
        Opens in binary mode.  The file is read as raw bytes, then each
        record is sliced according to the copybook layout and decoded
        from EBCDIC.
        """
    ...
    # ---------------------------------------------------------------
    # Cache management -- pickle for fast serialization
    # ---------------------------------------------------------------
