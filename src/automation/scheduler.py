...
def task_stream(task_queue, poll_interval=1.0):
    ...
    (C8) for injecting cancellation signals from the scheduler."""
    while True:
        try:
            try:
...
class TaskWorker(object):
    """Executes tasks from the shared queue via ``thread.start_new_thread()`` (D4)."""
...
class TaskScheduler(object):
    """Central scheduler using ``Queue.Queue`` (D3) for dispatch and
    ...
    def cancel_via_stream(self, stream, task_id):
        ...
        task_stream generator (C8 -- generator throw() API).

        In Python 2 the ``throw()`` method on generators accepts a
        three-argument form ``(type, value, traceback)``; Python 3
        requires keyword-only or positional arguments with different
        semantics for chained exceptions.
        """
        self._lock.acquire()
        try:
            task = self.registered_tasks.get(task_id)
