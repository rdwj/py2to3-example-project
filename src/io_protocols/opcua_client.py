...
class OpcUaClient:
    ...
    def connect(self):
        ...
        try:
            p = urllib.parse.urlencode({"RequestType": "CreateSession",
                                  "Timeout": int(self.timeout * 1000)})
            req = urllib.request.Request("%s?%s" % (self.endpoint, p))
            if self.auth_token:
    ...
    def disconnect(self):
        ...
        if self._sid:
            try:
                p = urllib.parse.urlencode({"RequestType": "CloseSession",
                                      "SessionId": self._sid})
                urllib.request.urlopen("%s?%s" % (self.endpoint, p), timeout=self.timeout)
            except urllib.error.URLError as e:
    ...
    def read_node(self, node_id, attr=ATTR_VALUE):
        ...
        url = "%s/read/%s?attr=%d&session=%s" % (
            self.endpoint, urllib.parse.quote(node_id, safe=""), attr, self._sid)
        try:
    ...
    def read_via_xmlrpc(self, gw_url, node_id):
        """Read via xmlrpc.client.ServerProxy."""
    ...
    def check_connection(self):
        """Check server reachability via http.client.HTTPConnection."""
        try:
        ...
        while self._polling:
            for sid, sub in self._subs.items():
                if not sub._items:
                    continue
                try:
                    p = urllib.parse.urlencode({"RequestType": "Publish",
                                          "SubscriptionId": sid,
                                          "SessionId": self._sid})
                    raw = urllib.request.urlopen("%s?%s" % (self.endpoint, p),
