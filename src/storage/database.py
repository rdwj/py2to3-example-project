...
# Py2 old-style class pickle registration removed — DataPoint is new-style in Py3

def _pickle_data_point(dp):
    """Pickle reducer for DataPoint."""
...
class DatabaseManager(object):
    ...
    def fetch_readings(self, tag, limit=100):
        """Most recent readings.  TEXT columns are ``str`` in Py3."""
        try:
    ...
    def get_raw_frame(self, reading_id):
        """Retrieve BLOB.  Returns ``bytes`` in Py3."""
        try:
            cur = self._conn.cursor()
